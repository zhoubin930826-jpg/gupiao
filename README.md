...
- 聚焦 A 股研究场景，围绕沪深主板、创业板、科创板和北交所组织候选池
- 在 `demo` 模式下即开即用
- 在 `AKShare` 模式下拉取 A 股真实快照、日线历史、财务摘要和事件层，并带基础重试与多来源兜底
...
3. 如果开启了 `AKShare`，后端会优先抓取 A 股真实快照和历史日线
4. A 股快照优先走东方财富轻量接口，失败时切到新浪，再兜底 AKShare 默认东财快照
5. 历史日线优先走东方财富；A 股失败时切腾讯和新浪
6. A 股补财务摘要和结构化事件
8. 根据信号引擎计算四维评分、推荐理由、风险提示和标签
9. 基于价格、量能和财务快照生成涨跌归因分析
10. A股会额外补北向资金、主力资金、龙虎榜活跃度摘要，以及前排候选的资金面分析
11. 刷新 DuckDB 中 A 股的快照、价格、行业热度、市场温度、推荐结果
12. 如果同步成功，把当前推荐结果写入推荐历史表
13. 复盘页会基于推荐历史和价格序列计算窗口收益
...
- 事件层第一版当前已接入结构化公告和业绩预告
- 暂时还没有把新闻、政策、北向、龙虎榜接进来
- 当前系统只服务 A 股场景，真实同步链路也只围绕 A 股展开
- 当前事件层以 A 股公告与业绩预告为主
...
- 默认已经接入 APScheduler，后端启动后会按计划自动执行 A 股同步
- 默认执行时间是：`A股 18:10`
