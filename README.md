...
- 左侧提供全局市场切换，当前支持 `A股（沪深） / 港股 / 美股`
- 在 `demo` 模式下即开即用
- 在 `AKShare` 模式下拉取 A 股、港股、美股的真实快照和日线历史，并带基础重试与多来源兜底
- 港股当前已补财务摘要和事件层；美股当前已补财务快照和财报日历类事件层
- 市场看板新增了 `市场状态 / 风格环境`，会先判断当前更适合顺势进攻、精选轮动还是谨慎防守
- A股看板额外补了 `关键指数` 和 `活跃样本宽度`，先帮助判断背景环境，再去看单票推荐
- A股看板额外补了 `北向资金 / 主力资金 / 龙虎榜活跃度` 摘要，帮助先判断当天资金环境
...
- [docs/PRODUCT_ROADMAP_TASKS.md](/Users/zhoubin/work/ideaWorkSpace/zhou/gupiao/docs/PRODUCT_ROADMAP_TASKS.md)
- [docs/DATA_UPGRADE_CHECKLIST.md](/Users/zhoubin/work/ideaWorkSpace/zhou/gupiao/docs/DATA_UPGRADE_CHECKLIST.md)
...
- 业务库：`SQLite`
- 行情分析库：`DuckDB`
- 数据采集：`AKShare` + `Yahoo Finance` 补充财务/财报日历 + 第三方公开站点兜底
- 测试：`pytest`
...
- 组合持仓
  ...
  - 查看单只持仓的成本、市值、权重、止损距离、目标距离、风险等级和风险提示
  - 展示行业暴露、风险持仓数、风险敞口和组合风险等级
  - 支持从交易计划一键转入组合持仓
- 自选池
...
- 真实模式下可拉取：
  ...
  - A 股 / 港股 / 美股结构化事件层摘要
- 多市场切换当前状态：
  - A股：支持真实 `AKShare` 同步，含财务和结构化事件层
  - 港股：支持真实 `AKShare` 同步，含财务快照、财报日历和盈利预测事件
  - 美股：支持真实 `AKShare` 同步，含财务快照、财报日历和股息日历事件
- 基于真实或示例数据统一生成：
  - 股票池快照
  - 价格序列
  - 推荐结果
...
3. 如果当前市场开启了 `AKShare`，后端会优先抓取该市场的真实快照和历史日线
4. A 股快照优先走东方财富轻量接口，失败时切到新浪，再兜底 AKShare 默认东财快照
5. 港股和美股快照走东方财富轻量分页接口，只抓前几页高成交标的，避免全市场扫表过慢
6. 历史日线优先走东方财富；A 股失败时切腾讯和新浪，港股 / 美股失败时切新浪历史接口
7. A 股补财务摘要和结构化事件；港股补财务摘要、财报日历和盈利预测；美股补财务快照、财报日历和股息日历
8. 根据信号引擎计算四维评分、推荐理由、风险提示和标签
9. 基于价格、量能和财务快照生成涨跌归因分析
10. A股会额外补北向资金、主力资金、龙虎榜活跃度摘要，以及前排候选的资金面分析
11. 刷新 DuckDB 中当前市场的快照、价格、行业热度、市场温度、推荐结果
12. 如果同步成功，把当前市场的推荐结果写入推荐历史表
13. 复盘页会基于当前市场的推荐历史和价格序列计算窗口收益
14. 你可以把高分候选进一步写成交易计划，记录计划价、止损价、目标价和执行状态
15. 同步完成后，系统会自动评估提醒，把计划触发、止损风险、目标接近、仓位过重和自选池异动写入提醒中心
16. 你可以把交易计划进一步转成组合持仓，跟踪仓位利用率、持仓盈亏和组合集中度
17. 你可以把关心的标的加入自选池，并继续维护跟踪状态和备注
18. 前端轮询任务状态并刷新页面
...
| `SCHEDULER_HK_OFFSET_MINUTES` | `15` | 港股任务相对基础时间的错峰分钟数 |
| `SCHEDULER_US_OFFSET_MINUTES` | `30` | 美股任务相对基础时间的错峰分钟数 |
...
- 涨跌归因分析第一版只基于价格、量能、估值和财务快照推断
- 它不是公告、新闻、政策级别的事实抓取结果
- 更适合回答“这只票现在为什么偏强/偏弱”，不是替代事件研究
- 事件层第一版当前已接入结构化公告、业绩预告、财报日历、股息日历和港股盈利预测
- 暂时还没有把新闻、政策、北向、龙虎榜接进来
- 左侧市场切换当前已经支持 `A股 / 港股 / 美股`
- 其中 `A股 / 港股 / 美股` 都已经能走真实同步链路
- 当前 `A股 / 港股 / 美股` 都已经有事件层；其中 A股偏公告与业绩预告，港股偏财报日历与盈利预测，美股偏财报日历与股息日历
- 推荐诊断会回答“这只票今天有没有进推荐、排第几、差在哪里、接下来盯什么”
...
- 组合风险等级、风险持仓数、风险敞口、最弱持仓
- 行业暴露和头部行业集中度
- 组合账户设置
- 持仓状态筛选
- 单只持仓的成本、市值、浮盈、实盈、权重、风控距离、风险等级和风险提示
...
- 查看数据源健康度、当前主源和回退链
- 查看事件层同步状态，包括覆盖数量、明确催化数量、事件条目数和本次命中来源
...
- 默认已经接入 APScheduler，后端启动后会按计划自动执行三个市场的市场同步
- 默认错峰时间是：`A股 18:10`、`港股 18:25`、`美股 18:40`
- 当前通过数据源适配层统一管理 `AKShare -> sample` 的回退链
- 页面会每 8 秒轮询一次运行中的任务
